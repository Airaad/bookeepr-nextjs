...
const BookDetail: React.FC<BookDetailProps> = ({ books }) => {
  ...
  const handleDelete = async () => {
    ...
    setOpen(false);
    try {
      const res = await axios.delete(`/api/delete-book/${books.id}`);
      if (res.status === 201) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1 sm:flex-row sm:gap-3 items-cetner mb-2">
          ...
          <span className="text-gray-600 text-medium text-sm sm:translate-y-[13px]">
            ({format(books.createdAt, "d MMMM, yyyy")})
          </span>
        </div>
        ...
        <div className="relative inline-block">
          ...
          {open && (
            <div className="absolute right-0 mt-2 w-32 bg-white border border-gray-200 rounded shadow-lg z-50">
              <button
                onClick={() => router.push(`/update-book-page/${books.id}`)}
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-6">
        <div className="flex-shrink-0">
          <Image
            src={
              books.coverId
                ? `https://covers.openlibrary.org/b/id/${books.coverId}-L.jpg`
